...
def save_benchmark_result(bench_result):
    plot_bar_chart(
        bench_result,
        of="mdx",
        label="MDX Execution time",
        title="Execution time by mdx query",
        out_file="img/pandas_spark_mdx_exec.png",
    )
    plot_bar_chart(
        bench_result,
        of="xmla",
        label="XMLA execution time",
        title="XMLA execution by query",
...
def plot_bar_chart(
    ...
    fig, ax = plt.subplots()
    ax.bar(ind - width / 2, pandas_exec_time, width, color="SkyBlue", label="Pandas")
    ax.bar(ind + width / 2, Spark_exec_time, width, color="IndianRed", label="Spark")
    ...
    ax.set_xticks(ind)
    ax.set_xticklabels(
        ["Q" + str(idx + 1) for idx, val in enumerate(bench_result["pandas"][of])]
    )
    ax.legend()
